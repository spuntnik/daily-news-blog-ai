...
type PostRow = {
  ...
  title: string;
  created_at: string;
  status: Stage; // stored in Supabase blog_posts.status
  sources: any;
};
...
export default function LibraryPage() {
  ...
  async function load() {
    ...
    try {
      ...
      // Ensure status is one of the pipeline stages (fallback to draft)
  ...
  async function onDragEnd(event: DragEndEvent) {
    ...
    if (PIPELINE.includes(overId as Stage)) {
      ...
      // dropping on another card -> infer that card stage
